...
from DDOKDDOK.models import Professor, TestRoom, Student, Log
from DDOKDDOK import db
from DDOKDDOK.views import auth_views
...
# watching_detail 페이지 부정행위 실시간 로그 딕셔너리 (test_room_id/student_id로 구분)
watching_detail_log_buffer = dict()
...
def make_ajax():
    ...
    start_date = datetime(int(date[0]), int(date[1]), int(
        date[2]), int(start[0]), int(start[1]))
    end_date = datetime(int(date[0]), int(date[1]), int(
        date[2]), int(end[0]), int(end[1]))
    ...
    for i in range(int(length)):
        ...
        image_data = img.read()
        student = Student(
            id=student_id, test_room_id=test_room_id, image=image_data)
...
def edit_list():
    ...
    for t in professor.test_room_set:
        test_room_list.append((t.id, t.state, t.title, str(
            t.start_date)[:16], str(t.end_date)[10:16]))
...
def watching(test_room_id):
    ...
    return render_template('test_room/watching.html', test_room_id=test_room_id, title=test_room.title,
                           start_date=str(test_room.start_date)[
                               :16], end_date=str(test_room.end_date)[10:16],
...
def student_login(test_room_id, student_id):
    ...
    if test_room is not None:
        ...
        student = Student.query.filter(
            Student.id == student_id, Student.test_room_id == test_room_id).first()
...
def watching_image_ajax(test_room_id):
    ...
    for g in global_images[test_room_id]:
        json_list.append({"student_id": g, "image": base64.b64encode(
            global_images[test_room_id][g]).decode()})
    return jsonify(json_list)
...
def watching_detail_log_ajax(test_room_id, student_id):
    ...
    json_list = [
        {"length": len(watching_detail_log_buffer[test_room_id][student_id])}]
...
def watching_detail_image_ajax(test_room_id, student_id):
    ...
    json_data = {"image": base64.b64encode(
        global_images[test_room_id][student_id]).decode()}
