# 프로그램 감시 + 키보드 감시   //파일이름 block 처럼 부정행위를 막아주는 파일이다. 
import os,signal
import keyboard
import time
...
# 운영체제 MAC 인 경우 아래 코드를 통해 비인가 sw 실행을 kill 시킨다.
def mac_killer():
    while True:
...
# 운영체제가 MAC 일 경우, 아래 코드를 통해 키보드 특정키 입력을 탐지하여 로그 기록 한다.
...
# 운영체제가 window 일 경우, 아래 코드를 통해 비인가 sw 실행을 kill 시킨다.
def win_killer(name):
    while True:
...
# 운영체제가 window 일 경우, 아래 코드를 통해 키보드 특정키 입력을 탐지하여 로그 기록 한다.
